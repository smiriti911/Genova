...
export function GeneSequence(props) {
  const {
    geneBounds,
    geneDetail,
    startPosition,
  ...
  return (
    <Card className="gap-0 border-none bg-neutral-900 py-0 shadow-sm">
      ...
      <CardContent className="pb-4">
        {geneBounds && (
          <div className="mb-4 flex flex-col">
            <div className="mb-2 flex flex-col items-center justify-between text-xs sm:flex-row">
              <span className="flex items-center gap-1 text-neutral-50">
            ...
            <div className="space-y-4">
              <div className="relative">
                ...
                >
                  ...
                  {/* Selected range */}
                  <div
                    className="absolute top-1/2 h-2 -translate-y-1/2 cursor-grab rounded-full bg-indigo-400 active:cursor-grabbing"
                    style={{
                  ...
                  {/* Start handle */}
                  <div
                    className="absolute top-1/2 flex h-6 w-6 -translate-x-1/2 -translate-y-1/2 cursor-grab items-center justify-center rounded-full border-2 border-indigo-400  bg-white shadow active:cursor-grabbing"
              ...
              <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                <div className="flex items-center gap-2">
                  <span className="text-xs text-neutral-50">Start:</span>
                  <Input
                    value={startPosition}
                    ...
                    inputMode="numeric"
                    pattern="[0-9]*"
                    className="h-7 w-full border-neutral-50 text-xs sm:w-28 bg-neutral-50"
                  />
                </div>
                <Button
                  size="sm"
                  ...
                  className="h-7 w-full cursor-pointer bg-indigo-400 text-xs text-white hover:bg-orange-400 sm:w-auto"
                >
                  {isLoading ? "Loading..." : "Load sequence"}
                </Button>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-neutral-50">End:</span>
                  <Input
                    value={endPosition}
                    ...
                    inputMode="numeric"
                    pattern="[0-9]*"
                    className="h-7 w-full border-neutral-50 bg-neutral-50 text-xs sm:w-28"
                  />
                </div>
              </div>
            </div>
        ...
        <div className="w-full rounded-md bg-neutral-50  p-3">
          {isLoading ? (
            ...
            <p className="text-center text-sm text-neutral-50/60">
        ...
        <div className="mt-3 flex items-center gap-4">
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs text-neutral-50/70">A</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="h-3 w-3 rounded-full bg-blue-600"></div>
            <span className="text-xs text-neutral-50/70">T</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="h-3 w-3 rounded-full bg-green-600"></div>
            <span className="text-xs text-neutral-50/70">G</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="h-3 w-3 rounded-full bg-amber-600"></div>
            <span className="text-xs text-neutral-50/70">C</span>
          </div>
        </div>
      </CardContent>
    </Card>
